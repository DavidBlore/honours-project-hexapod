...
for failure_scenario, failures in enumerate(scenarios):
	...
	for failed_legs in failures:
		...
		performances.append(max(fitness, 0.0))
		# performances.append(max(fitness / duration, 0.0))
	np.savetxt(os.path.join(os.path.dirname(__file__), "..", "output", "REF", "tripod-no-adapatation", f"tripod_{failure_scenario}.dat"), performances,'%3.5f')
